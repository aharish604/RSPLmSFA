...
public class MyTargetvsActualTargetActivity extends AppCompatActivity {
    RecyclerView recyclerViewAllDealerTarget;
    ArrayList<AllDealerTargetDTO> allDealerTargetDTOArrayList;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_my_targetvs_actual_target);
        //ActionBarView.initActionBarView(this, true, getString(R.string.title_my_targets_vs_actual_targets));

        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar);
        ConstantsUtils.initActionBarView(this, toolbar, true, getString(R.string.title_my_targets_vs_actual_targets), 0);
    ...
    private ArrayList<AllDealerTargetDTO> getAllDealerTargetDTOArrayList(){
        ...
        for (int i = 0; i < AllDealerTargetActivity.getMaterialList().size() ; i++) {
            ...
            allDealerTargetDTO.setTarget(AllDealerTargetActivity.getMyTargetvsActualList().get(i));
    ...
    public void onBackPressed() {
        AlertDialog.Builder builder = new AlertDialog.Builder(MyTargetvsActualTargetActivity.this, R.style.MyTheme);
        builder.setMessage(R.string.alert_exit_competition_information).setCancelable(false)
                .setPositiveButton(R.string.yes, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        dialog.cancel();
                        MyTargetvsActualTargetActivity.this.finish();
                    }
                })
