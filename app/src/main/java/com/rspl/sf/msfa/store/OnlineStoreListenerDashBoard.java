...
*/
/**
 * Created by e10860 on 5/29/2018.
 *//*


public class OnlineStoreListenerDashBoard implements OnlineODataStore.OpenListener {
    public static OnlineStoreListenerDashBoard instance;

    private final CountDownLatch latch = new CountDownLatch(1);
    OnlineODataStore store;
    Exception error;
    private OnlineStoreListenerDashBoard() {
    }

    public static OnlineStoreListenerDashBoard getInstance() {
        if (instance == null) {
            instance = new OnlineStoreListenerDashBoard();
        }
        return instance;
    }
    ...
    public void storeOpenError(ODataException e) {
            ...
            if(Constants.ErrorNo ==0) {
                try {
                    if(((ODataNetworkException.ErrorCode) e.errorCode).name().equalsIgnoreCase(Constants.NetworkError_Name)) {
                        ...
                        Throwable throwables = (e.getCause()).getCause().getCause();
        ...
        if(e!=null) {
            Constants.DashBroad_Error_Msg = e.getLocalizedMessage();
        }else{
            Constants.DashBroad_Error_Msg = "Unable to connect server.Please check your connection.";
        }
        Constants.IsOnlineStoreFailedDashBroad = true;
    }
    ...
    public void storeOpened(OnlineODataStore store) {
        ...
        latch.countDown();
        Constants.IsOnlineStoreFailedDashBroad = true;
        Constants.onlineStoreDashBroad = store;
        //latch.countDown();
        LogManager.writeLogInfo("Online store Dash Board opened successfully");
    }
