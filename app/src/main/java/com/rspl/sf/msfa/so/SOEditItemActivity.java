...
public class SOEditItemActivity extends AppCompatActivity  implements CheckBoxInterface {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // ActionBarView.initActionBarView(this, true, getString(R.string.title_sales_order_item));
        ...
        soItemAdapter = new SOItemAdapter(SOEditItemActivity.this, soItemBeanList);
...
//                    if(SOItem.getMaterialNo().equalsIgnoreCase(matItem.getMatCode())){
                    if(SOEditQuantityActivity.malterialHashmap.containsKey(matItem.getMatCode())){
                        matItem.setChecked(true);
                        matItem.setQuantity(SOEditQuantityActivity.malterialHashmap.get(matItem.getMatCode()));
                        matItem.setUom("");
...
//        SOItemBean soItemBean = soItemBeanList.get(position);
        ...
        if (isChecked) {
            SOEditQuantityActivity.malterialHashmap.put(soItemBean.getMatCode(),soItemBean.getQuantity());
            if (!tempSOItemBeanList.contains(soItemBean)) {
                tempSOItemBeanList.add(soItemBean);
            }
        } else {
            SOEditQuantityActivity.malterialHashmap.remove(soItemBean.getMatCode());
            if (tempSOItemBeanList.contains(soItemBean)) {
    ...
    public void onBackPressed() {
        AlertDialog.Builder builder = new AlertDialog.Builder(SOEditItemActivity.this, R.style.MyTheme);
        builder.setMessage(R.string.alert_exit_competition_information).setCancelable(false)
                .setPositiveButton(R.string.yes, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        dialog.cancel();
                        SOEditItemActivity.this.finish();
                    }
                })
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        inflater.inflate(R.menu.menu_expense_add, menu);
        return true;
    }
    ...
    private void onNext() {
        if (tempSOItemBeanList.size() > 0) {
            ...
            setResult(SOEditQuantityActivity.MATERIAL_REQUEST_CODE,soQt);
            finish();
