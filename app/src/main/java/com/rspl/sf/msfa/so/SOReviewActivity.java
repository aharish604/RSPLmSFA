...
public class SOReviewActivity extends AppCompatActivity implements UIListener, StoreStatusInterface {
    ...
    private void onItemDetails() {
        ...
        for (int i = 0; i < soItemBeansArrayList.size(); i++) {
            ...
            tvQty.setText(Constants.removeLeadingZero(singleItem.getSoQty()) + " " + singleItem.getUom());
            ...
            tvUnitPrice.setText(UtilConstants.removeLeadingZero(singleItem.getUnitPrice()) + " " + singleItem.getCurrency());
...
//            masterHeaderTable.put(Constants.MeansOfTranstyp, headerDetail.get(Constants.MeansOfTranstyp));
           ...
           // masterHeaderTable.put(Constants.Route, headerDetail.get(Constants.Route));
            ...
            if (isSimulated) {
                ...
                masterHeaderTable.put(Constants.TotalQuantity, headerDetail.get(Constants.TotalQuantity));
            } else {
            ...
            for (int itemIndex = 0; itemIndex < soItemBeansArrayList.size(); itemIndex++) {
                ...
                dbItemTable.put("Quantity", saveItem.getSoQty());
                ...
                double mDouSOItmQty = 0.0;
                try {
                    mDouSOItmQty = Double.parseDouble(saveItem.getSoQty());
                } catch (NumberFormatException e) {
                    mDouSOItmQty = 0.0;
                    e.printStackTrace();
                }
                sumQty = sumQty + mDouSOItmQty;
...
//                            singleItem.put(Constants.Freight, itemDesc.get("Freight"));
            ...
            masterHeaderTable.put(Constants.TotalQuantity, sumQty.toString());
            if (isComingFromChange)
            ...
            MyUtils.dialogConformButton(SOReviewActivity.this, msg, new DialogCallBack() {
                @Override
...
////                    }
            ...
            MyUtils.dialogConformButton(SOReviewActivity.this, "Data not found", new DialogCallBack() {
                @Override
                public void clickedStatus(boolean clickedStatus) {
//                    if(isComingFromChange)
...
//    }
    ...
    private void navtoPrevScreen() {
        ...
        Intent intentNavPrevScreen = new Intent(SOReviewActivity.this, CustomerDetailsActivity.class);
    ...
    private void navToSODetailScreen(boolean isSaveSuccess) {
        try {
            ...
            Intent intent = new Intent(SOReviewActivity.this, SalesOrderTabActivity.class);
...
//          intent.putExtra(Constants.comingFrom, mStrComingFrom);
            intent.putExtra(Constants.EXTRA_TAB_POS, 1);
...
//          intent.putExtra(Constants.EXTRA_SSRO_GUID, Constants.SOBundleExtras.getString(Constants.EXTRA_SSRO_GUID));
            ...
            if(isSaveSuccess)
                intent.putExtra(Constants.comingFromChange, false);
    ...
    public void onTaskCompleted(boolean storeStatus) {
        if (!storeStatus) {
            ...
            Constants.dialogSingleButton(SOReviewActivity.this, getString(R.string.online_store_error), new DialogCallBack() {
                @Override
                public void clickedStatus(boolean clickedStatus) {
                    clearConstants();
                }
    ...
    public void onRequestSuccess(int operation, String key) {
        ...
        if (operation == Operation.Create.getValue() && penReqCount == mIntPendingCollVal) {
            if (!Constants.isNetworkAvailable(SOReviewActivity.this)) {
                if (progressDialog != null) {
                    MyUtils.hideProgressDialog(progressDialog);
                }
                onNoNetwork();
                ...
                try {
                    OfflineManager.flushQueuedRequests(SOReviewActivity.this);
            ...
            Constants.dialogSingleButton(SOReviewActivity.this, popUpText, new DialogCallBack() {
                @Override
                public void clickedStatus(boolean clickedStatus) {
                    clearConstants();
                }
...
//        startActivity(intent);
    ...
    private void onNoNetwork() {
        Constants.dialogSingleButton(SOReviewActivity.this, getString(R.string.alert_sync_cannot_be_performed), new DialogCallBack() {
            @Override
            public void clickedStatus(boolean clickedStatus) {
                clearConstants();
            }
